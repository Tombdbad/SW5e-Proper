...
interface CombatState {
  ...
  // Combat actions
  initiateCombat: (player: Character, enemies: Omit<Combatant, "isPlayer">[]) => void;
  endCombat: () => void;
  nextTurn: () => void;
  rollInitiative: (dexModifier: number) => number;
  attackRoll: (targetId: number | string, attackValue: number, damageValue: number, isCritical: boolean) => void;
...
export const useCombat = create<CombatState>((set, get) => ({
  ...
  nextTurn: () => set((state) => {
    ...
    return {
      ...
      combatLog: [...state.combatLog, `${state.combatants[state.currentTurn + 1]?.name}'s turn`]
    };
  }),
  ...
  attackRoll: (targetId, attackValue, damageValue, isCritical) => set((state) => {
    // Find the target
    ...
    if (isCritical) {
      logMessage += `with a critical hit for ${damageValue} damage!`;
    ...
    if ((attackValue >= target.armorClass || isCritical) && damageValue > 0) {
      ...
      if (newHp === 0) {
        ...
        const remainingEnemies = updatedCombatants.filter(c => !c.isPlayer && c.hp > 0);
        if (remainingEnemies.length === 0) {
          return {
            ...state,
            combatants: updatedCombatants,
            combatLog: [...state.combatLog, logMessage, "Victory! All enemies defeated."],
            inCombat: false
          };
        }
        ...
        if (player && player.hp === 0) {
          return {
            ...state,
            combatants: updatedCombatants,
            combatLog: [...state.combatLog, logMessage, "Defeat! You have been defeated."],
            inCombat: false
          };
        }
      }
  ...
  damageRoll: (attackType, isCritical) => {
    ...
    switch (attackType) {
      case "melee":
        ...
        modifier = attacker.isPlayer && attacker.abilities ? 
