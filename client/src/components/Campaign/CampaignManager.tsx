...
import { saveCampaignState, fetchCampaignData } from "@/lib/api/campaign";

async function syncCampaignData(campaignId: string, data: any) {
  try {
    await saveCampaignState(campaignId, data);
  } catch (error) {
...
export default function CampaignManager({ campaign }: CampaignManagerProps) {
  ...
  const handleAddLocation = async () => {
    if (!newLocation.name || !newLocation.type || !newLocation.description)
      return;

    // Generate 3D map data from the description
    const locationData = await generateLocationFromDescription(
  ...
  const handleAddQuest = () => {
    if (
      ...
      !newQuest.objectives[0].description
    )
      return;
    ...
    <div className="space-y-8">
              ...
              <div className="space-y-4 py-2">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setNewNpc({ ...newNpc, species: e.target.value })
                    }
                  />
                </div>
          ...
          {campaign.npcs?.map((npc: any, index: number) => (
            <Card key={index} className="bg-gray-700">
              ...
              <CardContent className="py-2">
                <p>
                  <strong>Species:</strong> {npc.species}
                </p>
                <p>
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <Dialog>
            <DialogTrigger asChild>
              <Button size="sm">Add Quest</Button>
            </DialogTrigger>
